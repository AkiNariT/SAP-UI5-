...
To add functionality to the control, we can provide meta information via a static property named `metadata`OpenUI5. This can automatically create setter and getter methods along with other convenience functions that can be used within the application. We'ill leave the `metadata` property empty for now.
...
[init\(\) Method](../07_Developing_Controls/init-method-6d6b5bd.md "The init() method can be used to set up, for example, internal variables or subcontrols of a composite control.")

[API Reference: `sap.ui.core.ControlRenderer`](https://ui5.sap.com/#/api/sap.ui.core.ControlRenderer)

[Renderer Object](../07_Developing_Controls/renderer-object-c9ab345.md "The renderer object is responsible for creating the HTML structure for the control.")
...
[Event Handler Methods](../07_Developing_Controls/event-handler-methods-bdf3e98.md "Event handler methods are invoked when an event occurs. Method names starting with on are reserved for event handler methods.")
